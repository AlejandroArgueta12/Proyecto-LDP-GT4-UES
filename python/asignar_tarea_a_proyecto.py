...
class AsignarTareaAProyecto(PantallaBase):
    def __init__(self, ventana_padre=None, panel_retorno=None):
        ...
        tk.Button(self.menu, text="✔  Asignar a proyectos", bg=self.COLOR_MENU, fg=self.COLOR_TEXTO, activebackground=self.COLOR_MENU, activeforeground=self.COLOR_TEXTO, bd=0, anchor="w", justify="left", cursor="hand2", font=("Arial", 16, "bold"), padx=20, pady=20).pack(fill="x", pady=20)
        ...
        tk.Label(self.frame_central, text="ID de Proyecto", bg=self.COLOR_FONDO, fg=self.COLOR_TEXTO, font=("Arial", 18, "bold")).pack(pady=(0, 15))
        self.entry_proyecto = tk.Entry(self.frame_central, width=30, font=("Arial", 12))
        self.entry_proyecto.pack(ipady=5, pady=(0, 40))

        tk.Button(self.frame_central, text="Asignar Tarea", bg="#2f6df6", fg="white", font=("Arial", 12, "bold"), cursor="hand2", padx=25, pady=8, relief="flat", command=self.asignar).pack()

    def asignar(self):
        id_tarea = self.entry_tarea.get().strip().upper()
        id_proyecto = self.entry_proyecto.get().strip().upper()
        if not id_tarea or not id_proyecto:
            messagebox.showwarning("Advertencia", "Complete todos los campos.")
            return
        
        exito, msj = db.asignar_tarea_proyecto(id_tarea, id_proyecto)
        if exito:
